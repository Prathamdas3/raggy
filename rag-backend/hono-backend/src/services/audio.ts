...
const AudioFunc = async (
	job: Job<{ answerId: string; type: "chat" | "message" }>,
) => {
	...
	if (type === "chat") {
		...
		if (error || !data?.content.trim()) {
			throw new Error(
				"Failed to fetch the content of the message for the audio generation",
			);
		}
...
export const AudioWorker = new Worker("audio", AudioFunc, {
	connection: redis,
});
