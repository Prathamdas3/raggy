...
const SummaryFunc = async (job: Job) => {
	...
	const { data, error: SummaryError } = await tryCatch(
		chain.invoke({ context: content }),
	);

	if (SummaryError) {
		logger.error(
			`Failed to generate the summarized docs:${SummaryError.message}`,
		);
		throw new Error("Failed to generate the summarized docs");
	}

	const { data: updatedSummaryList, error: SummarySaveError } = await tryCatch(
		updateDocs(docId, data),
	);
	if (SummarySaveError) {
	...
	const { error: AudioQueueError } = await tryCatch(
		AddToAudioQueue({ answerId: docId, type: "chat" }),
	);

	if (AudioQueueError) {
		logger.error(
			"Failed to add the summary id in the audio queue for further processing",
		);
...
export const SummaryWorker = new Worker("summary", SummaryFunc, {
	connection: redis,
});
