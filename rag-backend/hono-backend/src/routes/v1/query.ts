...
router
	.post(
		...
		async (c) => {
			...
			const { data: details, error: QueryDbStoring } = await tryCatch(
				createMessage({ sender: "user", chat_id: chatId, content: question }),
			);
			...
			if (QueryDbStoring) {
				...
				return c.json(
					error(
						"Failed to store the data, Please try again",
			...
			const { error: QueryAddingError } = await tryCatch(
				AddToQueryQueue({
					chatId,
					userId,
					question,
					questionId: details[0].id,
				}),
			);
			...
			return c.json(
				success({
					message: "Successfully submited the qeustion for answer generation",
					question_id: details[0].id,
				}),
			);
		},
	)
	.get("/:chat_id/:question_id", async (c) => {
		...
		if (!question_id.trim() || !chat_id.trim()) {
			...
			return c.json(
				error("No question id or chat id found", "Invalid Input"),
				400,
			);
		}

		const { data, error: getAnswerError } = await tryCatch(
			getAnswerMessage(chat_id, question_id),
		);

		if (getAnswerError) {
			logger.error(
				`Failed to fetch the answer for the chat_id:${chat_id} and question_id:${question_id}`,
			);
			return c.json(
				error("Failed to fetch the answer", "Internal Server Error"),
				500,
			);
		}
		...
		return c.json(
			success({ content: data?.content, answer_id: data?.id }),
			200,
		);
	});
