...
router
	.use(async (c, next) => {
		...
		if (!contentType.startsWith("multipart/form-data")) {
			return c.json(
				error("Only multipart/form-data is allowed", "Invalid Input"),
				400,
			);
		}
		await next();
	...
	.post(
		bodyLimit({
			...
			onError: (c) => {
				return c.json(
					error("FileSize should be below 5mb", "Invalid Input"),
					400,
				);
			},
		}),
		...
		validator("form", (value, c) => {
			...
			if (!allowedTypes.includes(parsed.data.file.type)) {
				return c.json(
					error(
						`Invalid file type, file must be one of these types ${allowedTypes.join(",")}`,
						"Invalid Input",
					),
					400,
				);
		...
		async (c) => {
			...
			const { data: oldDoc, error: oldDocError } = await tryCatch(
				getDocsByChatId(chat_id),
			);

			if (oldDocError || !oldDoc) {
				return c.json(
					error(
						"This chat already has a docs attach to it, every chat can have only one doc attach to it",
					),
				);
			}
			...
			const { error: SaveFileError } = await tryCatch(
				fs.promises.writeFile(filePath, Buffer.from(arrayBuffer)),
			);
			if (SaveFileError) {
				if (filePath) {
					try {
