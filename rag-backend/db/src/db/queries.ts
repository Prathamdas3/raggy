...
export const updateMessage = async (
	...
	return await database
		.update(messages)
		.set({ content: newMessage, parent_message_id: messages.id })
		.where(
			and(
				eq(messages.chat_id, chatId),
				parentId === null
					? isNull(messages.parent_message_id)
					: eq(messages.parent_message_id, parentId),
			),
		);
};
...
export const getMessagesByParentId = async (
	...
	return await database
		.select({
			...
			parent_id: messages.parent_message_id,
...
export const getAnswerMessage = async (chatId: string, questionId: string) => {
	return (
		await database
			...
			.from(messages)
			.where(
				and(eq(messages.chat_id, chatId), eq(messages.question_id, questionId)),
			)
	).find(
		({ question_id, chat_id }) =>
			question_id === questionId && chat_id === chatId,
	);
};
...
export const getAnswerById = async (answerId: string) => {
	return (
		await database
			.select({ content: messages.content, id: messages.id })
			.from(messages)
			.where(and(eq(messages.id, answerId), eq(messages.sender, "llm")))
	).find(({ id }) => id === answerId);
};
