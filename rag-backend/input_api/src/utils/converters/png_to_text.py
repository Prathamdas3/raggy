...
async def extract_text_from_image(image_file_path: str, file_type: str) -> str:
    ...
    logger.info(f"Starting text extraction from image. File: {image_file_path}, Type: {file_type}")
    ...
    try:
        ...
        if not image_file_path.is_file():
            ...
            raise APIError(f"Path is not a valid file: {image_file_path}", status_code=400)
        ...
        try:
            ...
            if not extracted_text:
                logger.warning(f"Extracted text is empty after stripping: {png_file_path}")
                raise APIError(
                    "No readable text found in image",
                    status_code=400
                )
            
            logger.info(f"Text extraction successful. Text length: {len(extracted_text)} characters")
