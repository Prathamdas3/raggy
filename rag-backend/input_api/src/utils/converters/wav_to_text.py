...
async def transcribe_audio(audio_file: str) -> str:
    ...
    try:
        ...
        if not audio_file_path.is_file():
            ...
            raise APIError(
                f"Path is not a valid file: {audio_file_path}", status_code=400
            )
        ...
        try:
            ...
            raise APIError(
                "Failed to initialize Whisper model", status_code=500, details=str(e)
            )
        ...
        try:
            ...
            transcription_result = await asyncio.to_thread(
                whisper_model.transcribe, str(audio_file_path), language="en"
            )

            if not transcription_result or "text" not in transcription_result:
                logger.error("Invalid transcription result format")
                raise APIError("Transcription returned invalid result", status_code=500)

            transcribed_text = transcription_result["text"].strip()

            if not transcribed_text:
                logger.warning("Transcription resulted in empty text")
                raise APIError("No speech detected in audio file", status_code=400)
            ...
            raise APIError(
                "Failed to transcribe audio", status_code=500, details=str(e)
            )
