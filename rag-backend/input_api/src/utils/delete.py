...
async def delete_file(file_path: Union[str, Path]) -> None:
    ...
    try:
        ...
        if not file_path.is_file():
            ...
            raise APIError(
                f"Path is a directory, not a file: {file_path}", status_code=400
            )
        ...
        if not os.access(file_path.parent, os.W_OK):
            ...
            raise APIError(
                "No permission to delete file (permission denied)", status_code=403
            )
        ...
        logger.error(
            f"Attempted to delete a directory as file: {file_path}: {str(ide)}"
        )
        raise APIError(
            "Cannot delete - path is a directory", status_code=400, details=str(ide)
        )
        ...
        raise APIError(
            "OS error - unable to delete file", status_code=500, details=str(ose)
        )
