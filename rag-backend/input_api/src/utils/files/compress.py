...
async def compress_video(file_path: str) -> str:
    ...
    try:
        ...
        try:
            ...
            raise APIError(
                "Video compression timed out - file may be too large", status_code=500
            )
            ...
            if compressed_file_path and compressed_file_path.exists():
                try:
                    ...
                    logger.info(
                        f"Cleaned up failed compressed file: {compressed_file_path}"
                    )
                except Exception as cleanup_e:
                    logger.error(f"Failed to cleanup compressed file: {str(cleanup_e)}")

            raise APIError("Video compression failed", status_code=500, details=str(e))

        # ===== Verify Compressed File =====
        if not compressed_file_path.exists():
            logger.error(f"Compressed file was not created: {compressed_file_path}")
            raise APIError(
                "Compressed file was not created successfully", status_code=500
            )
        ...
        if compressed_size == 0:
            ...
            raise APIError(
                "Compressed file is empty - compression failed", status_code=500
            )
        ...
        if compressed_file_path and compressed_file_path.exists():
            try:
                compressed_file_path.unlink()
                logger.info(
                    f"Cleaned up compressed file after error: {compressed_file_path}"
                )
            except Exception as cleanup_e:
                logger.error(f"Failed to cleanup compressed file: {str(cleanup_e)}")

        raise APIError(
            "Unexpected error during video compression", status_code=500, details=str(e)
        )
