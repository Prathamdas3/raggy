...
def split_text_task(self, text: str, user_id: str, chat_id: str) -> dict:
    ...
    """
    logger.info(f"Text splitting task started. User: {user_id}, Chat: {chat_id}, Text length: {len(text) if text else 0}")
    ...
    try:
        ...
        try:
            ...
            if not split_texts:
                logger.warning(f"Text splitter returned empty list for text of length: {len(text)}")
                return {
                    "status": "error",
        ...
        # ===== Return Success =====
        logger.info(f"Text splitting task completed successfully. Chunks: {len(chunks_with_metadata)}")
        return {
            "status": "success",
            "message": "Text split successfully with metadata",
            "code": 200,
            "data": {
