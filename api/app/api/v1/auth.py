...
from app.core import config, get_logger, ServiceException, AppException
...
async def get_user_id(request: Request, session: SessionDep) -> RefreshTokenUserId:
    ...
    old_user: RefreshTokenUserId = await user.get_user_id_from_refresh_token(
        request=request
    )
...
def create_auth_tokens(payload: Tokens) -> tuple[str, str]:
    ...
    if not access_token or not refresh_token:
        raise ServiceException("Failed to generate tokens.")
...
async def handle_signup(
    ...
    user = await auth.user_signup(data=data)
...
async def handle_signin(
    ...
    user = await auth.user_signin(data=data)
    access_token, refresh_token = create_auth_tokens(
        Tokens(user_id=str(user.id), email=user.email)
    )
