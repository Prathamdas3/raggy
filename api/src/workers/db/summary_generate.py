...
async def request_summary_generation(user_id: str, chat_id: str, summary: str):
    try:
        async with httpx.AsyncClient(timeout=int(SUMMARY_API_TIMEOUT)) as client:
            payload = {"user_id": user_id, "chat_id": chat_id, "original_text": summary}
            url = f"{SUMMARY_API_URL}/summary"
            response = await client.post(
                url, json=payload, headers={"Content-Type": "application/json"}
            )
            logger.info(
                f"Summary API responded with status code: {response.status_code}"
            )
            if response.status_code == 202:
                ...
                logger.error(
                    f"Summary API error ({response.status_code}): {error_detail}"
                )
                ...
                logger.warning(
                    f"Unexpected status code from Summary API: {response.status_code}"
                )
                return {
                    "status": "success",
        ...
        logger.error(
            f"Request to Summary API timed out after {SUMMARY_API_TIMEOUT} seconds"
        )
        raise APIError(
...
def send_request_for_summary_generation(self, user_id: str, chat_id: str, summary: str):
    try:
        ...
        logger.info(
            f"Initiating summary generation request for user_id: {user_id}, chat_id: {chat_id}"
        )
