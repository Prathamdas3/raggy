...
async def request_vectorstore(chunks: List[ChunkData]):
    try:
        async with httpx.AsyncClient(timeout=int(SUMMARY_API_TIMEOUT)) as client:
            ...
            logger.info(
                f"Vector Store API responded with status code: {response.status_code}"
            )
            if response.status_code == 202:
                ...
                logger.error(
                    f"Vector Store API error ({response.status_code}): {error_detail}"
                )
                ...
                logger.warning(
                    f"Unexpected status code from Vector Store API: {response.status_code}"
                )
                return {
                    "status": "success",
        ...
        logger.error(
            f"Request to Vector Store API timed out after {SUMMARY_API_TIMEOUT} seconds"
        )
        raise APIError(
        ...
        raise APIError(
            "Failed to connect to Vector Store API", status_code=503, details=str(ce)
        )
    except Exception as e:
        logger.exception(
            f"Unexpected error while requesting Vector Store API: {str(e)}"
        )
        raise APIError(
