...
def extract_text_from_wav(
    self, file_path: str, file_type: str, user_id: str = None, chat_id: str = None
) -> dict:
    """
    ...
    logger.info(
        f"File: {file_path}, Type: {file_type}, User: {user_id}, Chat: {chat_id}"
    )
    ...
    try:
        ...
        if split_task_id:
            ...
            logger.warning(
                f"Transcription succeeded but splitting failed: {split_error}"
            )
            return {
