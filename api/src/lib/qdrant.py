...
HUGGINGFACE_DEVICE = os.getenv("HUGGINGFACE_DEVICE", "cpu")  # or "cuda" for GPU
            ...
            with cls._lock:
                ...
                if cls._qdrant_client is None:
                    try:
                        ...
                        logger.error(f"QDRANT_HOST={QDRANT_HOST}")
                        logger.error(f"QDRANT_PORT={QDRANT_PORT}")
            ...
            with cls._lock:
                if cls._embeddings is None:
                    try:
                        ...
                        logger.info(f"Embedding Model: {HUGGINGFACE_MODEL}")
                        ...
                        if embedding_dim != QDRANT_VECTOR_SIZE:
                            logger.warning(
                                f"⚠ Embedding dimension ({embedding_dim}) doesn't match "
                                f"configured vector size ({QDRANT_VECTOR_SIZE}). "
    ...
    def initialize_collection(cls) -> None:
        ...
        try:
            ...
            if QDRANT_COLLECTION_NAME not in collection_names:
                logger.info(
                    f"Collection '{QDRANT_COLLECTION_NAME}' does not exist. Creating..."
                )
