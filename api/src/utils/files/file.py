...
async def handle_file(chat_id: str, user_id: str, file: UploadFile = File(...)) -> dict:
    """
    ...
    try:
        ...
        logger.info(
            f"Received file: {file.filename} from user: {user_id}, chat: {chat_id}"
        )
        ...
        if not is_supported:
            logger.warning(
                f"Unsupported file type: {mime_type} for file: {file.filename}"
            )
            raise HTTPException(
        ...
        if mime_type in ALLOWED_IMAGE_TYPES:
            try:
                ...
                logger.info(
                    f"Queued OCR extraction task {task.id} for image file: {file.filename}"
                )
                ...
                logger.error(
                    f"Failed to queue image OCR task for {file.filename}: {str(e)}"
                )
        ...
        # Audio Files
            ...
            try:
                ...
                logger.info(
                    f"Queued transcription task {task.id} for audio file: {file.filename}"
                )
                ...
                logger.error(
                    f"Failed to queue audio transcription task for {file.filename}: {str(e)}"
                )
        ...
        # Video Files
            ...
            try:
                ...
                task = extract_text_from_wav.delay(str(file_path), mime_type)
                logger.info(
                    f"Queued transcription task {task.id} for video file: {file.filename}"
                )
                ...
                logger.error(
                    f"Failed to queue video transcription task for {file.filename}: {str(e)}"
                )
        ...
        # Document Files
            ...
            try:
                ...
                logger.error(
                    f"Failed to queue document extraction task for {file.filename}: {str(e)}"
                )
