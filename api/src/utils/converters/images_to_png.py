...
async def convert_image_to_png(file_path: str, file_type: str) -> str:
    ...
    logger.info(
        f"Starting image conversion to PNG. File: {file_path}, Type: {file_type}"
    )
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Image successfully converted and saved to PNG: {png_file_path}"
            )
            ...
            raise APIError(
                "Invalid or corrupted image file", status_code=400, details=str(ose)
            )
            ...
            raise APIError(
                "Failed to convert image to PNG format", status_code=500, details=str(e)
            )

        # ===== Verify PNG File =====
        if not png_file_path.exists():
            logger.error(f"PNG file was not created: {png_file_path}")
            raise APIError("PNG file was not created successfully", status_code=500)
        ...
        if file_size == 0:
            ...
            raise APIError("PNG file created but is empty", status_code=500)
        ...
        raise APIError(
            "Unexpected error during image conversion", status_code=500, details=str(e)
        )
