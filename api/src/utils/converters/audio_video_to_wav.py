...
async def convert_audio_video_to_wav(file_path: str, file_type: str) -> str:
    ...
    logger.info(
        f"Starting conversion to WAV using FFmpeg. File: {file_path}, Type: {file_type}"
    )
    ...
    try:
        ...
        try:
            temp_file = tempfile.NamedTemporaryFile(
                suffix=".wav", delete=False, dir="temp"
            )
            ...
            raise APIError(
                "Failed to create temporary WAV file", status_code=500, details=str(e)
            )
        ...
        try:
            ...
            raise APIError(
                "Audio conversion timed out - file may be too large", status_code=500
            )
            ...
            raise APIError(
                "Failed to convert file to WAV format", status_code=500, details=str(e)
            )

        # ===== Verify WAV File =====
        if not os.path.exists(wav_file_path):
            logger.error(f"WAV file was not created: {wav_file_path}")
            raise APIError("WAV file was not created successfully", status_code=500)
        ...
        if file_size == 0:
            ...
            raise APIError("WAV file created but is empty", status_code=500)
        ...
        raise APIError(
            "Unexpected error during audio conversion", status_code=500, details=str(e)
        )
