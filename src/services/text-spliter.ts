...
const SplitingFunc = async (job: Job) => {
	...
	const { data: texts, error: textsError } = await tryCatch(
	...
	const { data, error: SaveDocsError } = await tryCatch(
		createDocs({
			title,
			user_id: userId,
			chat_id: chatId,
	...
	const { error: SummaryQueueError } = await tryCatch(
		AddToSummaryQueue({ content: newTexts, docId: data[0].chat_id }),
	);
	...
	const { error: EmbeddingQueueError } = await tryCatch(
		AddToEmbedingQueue(newTexts),
	);
...
export const TextSplitingWorker = new Worker("text-spliter", SplitingFunc, {
	connection: redis,
});
